...
- **用户端**：密码/短信双登录、商品搜索与分类浏览、购物车、收藏、下单（跨商家自动拆单）、收银台（模拟支付/微信支付）、确认收货、修改密码、注销账户
- **商家端**：入驻申请与审核、商品管理（图片上传、分类级联选择、上下架）、店铺订单管理、发货、经营数据看板（销售额/订单统计）
- **管理后台**：数据看板（12 项统计指标）、用户管理、商家审核、商品管控、全平台订单管理
- **技术亮点**：乐观锁防超卖、按商家拆单的事务一致性、超时订单自动关闭（定时任务）、微信支付 V3 集成（验签 + 幂等回调）、图形验证码 + 短信验证码、雪花 ID 字符串化防精度丢失

## 订单状态机

```
待支付(0) ──支付──> 已支付/待发货(1) ──商家发货──> 已发货/待收货(4) ──确认收货──> 已完成(3)
   │
   └──用户取消/超时关闭──> 已取消(2)
```
...
| 服务 | 配置项 | 说明 |
|------|--------|------|
| 阿里云短信 | `aliyun.sms.*` | 需要 AccessKey，未配置时使用模拟模式（验证码打印到后端控制台，也可从 Redis `sms:code:phone:*` 读取） |
| 微信支付 | `wechat.pay.*` | 需要商户号和证书，未配置时收银台使用模拟支付 |

### 图片上传

上传的商品图片保存在 `goods-market-backend/uploads/images/` 目录（`app.upload-dir` 可配置），
通过 `/api/images/upload/**` 访问。
